#!/usr/bin/env python3
"""
Training script for hybrid LSTM energy consumption prediction model.

Usage:
    python lstm/train.py                                    # defaults
    python lstm/train.py --name exp1 --seq-length 48        # overrides
    python lstm/train.py --utility STEAM --epochs 100       # different utility
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train hybrid LSTM energy model")
...
def main():
    ...
    if args.utility:
        ...
        cfg.name = f"energy_lstm_{args.utility.lower()}"
    ...
    print("=" * 60)
    print(f"Hybrid LSTM Energy Prediction — {cfg.name}")
    print(f"Output: {run_dir}")
    print(f"Utility: {cfg.data.utility_filter}")
    print(f"Seq length: {cfg.data.seq_length} ({hours_covered:.1f}h)")
    print(f"LSTM: hidden_size={cfg.lstm.hidden_size}, layers={cfg.lstm.num_layers}")
