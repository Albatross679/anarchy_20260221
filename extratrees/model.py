"""
Extra Trees model for energy consumption prediction with enhanced feature engineering.

Functions:
    engineer_features -- add lag, rolling, and interaction features
    create_model      -- instantiate ExtraTreesRegressor from config
...
from extratrees.config import ExtraTreesDataConfig, ExtraTreesParams, TensorBoardConfig, log_system_metrics_to_tb
...
def create_model(
    params: ExtraTreesParams, seed: int = 42
) -> Tuple[ExtraTreesRegressor, dict]:
    """Create ExtraTreesRegressor from config params."""
...
def train_model(
    ...
    y_test: pd.Series,
    fit_params: dict,
    params: Optional[ExtraTreesParams] = None,
    run_dir: Optional[Path] = None,
    tb_cfg: Optional[TensorBoardConfig] = None,
) -> ExtraTreesRegressor:
    """Fit Extra Trees (no early stopping). Logs final metrics to TensorBoard."""
    if tb_cfg is None:
...
def save_model(model: ExtraTreesRegressor, path: str | Path) -> None:
...
def load_model(path: str | Path) -> ExtraTreesRegressor:
