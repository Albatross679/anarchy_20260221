#!/usr/bin/env python3
"""
Training script for Extra Trees energy consumption prediction model.

Usage:
    python extratrees/train.py                                    # defaults
    python extratrees/train.py --name exp1 --n-estimators 100     # overrides
    python extratrees/train.py --utility STEAM --max-depth 30     # different utility
"""
...
from extratrees.config import (
    EnergyExtraTreesConfig,
    setup_console_logging,
    setup_output_dir,
)
from extratrees.model import (
    create_model,
    engineer_features,
    evaluate_model,
    get_predictions,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Extra Trees energy model")
...
def main():
    ...
    if args.n_estimators is not None:
        cfg.et.n_estimators = args.n_estimators
    if args.max_depth is not None:
        cfg.et.max_depth = args.max_depth
    if args.min_samples_split is not None:
        cfg.et.min_samples_split = args.min_samples_split
    if args.min_samples_leaf is not None:
        cfg.et.min_samples_leaf = args.min_samples_leaf
    if args.max_features is not None:
        cfg.et.max_features = args.max_features
    ...
    print("=" * 60)
    print(f"Extra Trees Energy Prediction -- {cfg.name}")
    ...
    try:
        ...
        print("\n--- Training ---")
        model, fit_params = create_model(cfg.et, cfg.seed)
        model = train_model(
            model, X_train, y_train, X_test, y_test, fit_params,
            params=cfg.et, run_dir=run_dir, tb_cfg=cfg.tensorboard,
        )
