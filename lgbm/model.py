"""
LightGBM model for energy consumption prediction with enhanced feature engineering.

Functions:
    engineer_features -- add lag, rolling, and interaction features
    create_model      -- instantiate LGBMRegressor from config
    train_model       -- fit with early stopping
    evaluate_model    -- compute RMSE/MAE/R²/MAPE + SHAP + plots
    save_model        -- persist to native txt format
    load_model        -- load from native txt format
...
from lgbm.config import LGBMDataConfig, LightGBMParams, TensorBoardConfig, log_system_metrics_to_tb
...
def _make_tb_callback(
    ...
    """Create a LightGBM callback that logs per-round metrics to TensorBoard."""
    state = {"start_time": None, "last_time": None, "y_val": np.asarray(y_val), "X_val": X_val}
    ...
    def callback(env):
        ...
        round_time = now - state["last_time"]
        state["last_time"] = now

        # Log built-in eval metrics every round
        train_rmse = None
        val_rmse = None
        for dataset, metric, value, _ in env.evaluation_result_list:
            writer.add_scalar(f"{dataset}/{metric}", value, iteration)
            if dataset == "train" and metric == "rmse":
                train_rmse = value
            if dataset == "validation" and metric == "rmse":
                val_rmse = value

        # Combined train/val loss for parity with neural models
        if train_rmse is not None and val_rmse is not None:
            writer.add_scalars("loss", {"train": train_rmse, "val": val_rmse}, iteration)

        # Compute MAE / R² on val set at configurable interval
        if iteration % log_every == 0:
            y_pred_val = env.model.predict(state["X_val"], num_iteration=iteration + 1)
            y_v = state["y_val"]
            val_mae = float(np.mean(np.abs(y_v - y_pred_val)))
            ss_res = np.sum((y_v - y_pred_val) ** 2)
            ss_tot = np.sum((y_v - np.mean(y_v)) ** 2)
            val_r2 = float(1 - ss_res / ss_tot) if ss_tot > 0 else 0.0

            writer.add_scalar("metrics/val_rmse", float(np.sqrt(np.mean((y_v - y_pred_val) ** 2))), iteration)
            writer.add_scalar("metrics/val_mae", val_mae, iteration)
            writer.add_scalar("metrics/val_r2", val_r2, iteration)

            log_system_metrics_to_tb(writer, tb_cfg, iteration)

        # Timing
        writer.add_scalar("time/wall_clock_seconds", elapsed, iteration)
        writer.add_scalar("time/round_seconds", round_time, iteration)

    callback.order = 100  # run after built-in callbacks
    return callback
...
def create_model(
    params: LightGBMParams, seed: int = 42
) -> Tuple[LGBMRegressor, dict]:
    """Create LGBMRegressor from config params.

    Returns (model, fit_params) where fit_params contains callback settings.
    """
    kwargs = dict(
    ...
    if params.early_stopping_rounds > 0:
        ...
        fit_params["callbacks"] = [
            early_stopping(params.early_stopping_rounds),
            log_evaluation(params.verbose),
        ]
...
def train_model(
    ...
    # Add TensorBoard callback to existing callbacks
    callbacks = fit_params.get("callbacks", [])
    if writer is not None:
        callbacks.append(_make_tb_callback(
            writer, tb_cfg,
...
def evaluate_model(
    ...
    metrics = {
        ...
        "mape_pct": mape,
        "n_test": len(y_test),
        "n_trees_used": model.best_iteration_ if model.best_iteration_ else model.n_estimators,
...
def save_model(model: LGBMRegressor, path: str | Path) -> None:
    """Save model in LightGBM native txt format."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    model.booster_.save_model(str(path))


def load_model(path: str | Path) -> LGBMRegressor:
    """Load model from LightGBM native txt format."""
