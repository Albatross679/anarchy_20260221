"""
Random Forest model for energy consumption prediction with enhanced feature engineering.

Functions:
    engineer_features -- add lag, rolling, and interaction features
    create_model      -- instantiate RandomForestRegressor from config
...
from rf.config import RFDataConfig, RandomForestParams, TensorBoardConfig, log_system_metrics_to_tb
...
def create_model(
    params: RandomForestParams, seed: int = 42
) -> Tuple[RandomForestRegressor, dict]:
    """Create RandomForestRegressor from config params.
...
def train_model(
    ...
    """Fit Random Forest (no early stopping). Logs final metrics to TensorBoard."""
    if tb_cfg is None:
