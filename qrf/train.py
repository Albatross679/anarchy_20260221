#!/usr/bin/env python3
"""
Training script for Quantile Regression Forest energy consumption prediction model.

QRF provides prediction intervals via quantile estimation.

Usage:
    python qrf/train.py                                    # defaults
    python qrf/train.py --name exp1 --n-estimators 100     # overrides
    python qrf/train.py --utility STEAM --max-depth 30     # different utility
"""
...
from qrf.config import (
    EnergyQRFConfig,
    setup_console_logging,
    setup_output_dir,
)
from qrf.model import (
    create_model,
    engineer_features,
    evaluate_model,
    get_predictions,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Quantile Regression Forest energy model")
...
def main():
    ...
    if args.n_estimators is not None:
        cfg.qrf.n_estimators = args.n_estimators
    if args.max_depth is not None:
        cfg.qrf.max_depth = args.max_depth
    if args.min_samples_split is not None:
        cfg.qrf.min_samples_split = args.min_samples_split
    if args.min_samples_leaf is not None:
        cfg.qrf.min_samples_leaf = args.min_samples_leaf
    if args.max_features is not None:
        cfg.qrf.max_features = args.max_features
    ...
    print("=" * 60)
    print(f"Quantile Regression Forest Energy Prediction -- {cfg.name}")
    ...
    try:
        ...
        print("\n--- Training ---")
        model, fit_params = create_model(cfg.qrf, cfg.seed)
        model = train_model(
            model, X_train, y_train, X_test, y_test, fit_params,
            params=cfg.qrf, run_dir=run_dir, tb_cfg=cfg.tensorboard,
        )
        ...
        df_preds = get_predictions(model, df, feature_cols, quantiles=cfg.qrf.quantiles)
