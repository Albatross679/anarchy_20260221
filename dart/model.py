"""
DART model for energy consumption prediction with enhanced feature engineering.

DART (Dropouts meet Multiple Additive Regression Trees) extends XGBoost with
dropout regularization. Early stopping is disabled by default as it is unreliable
with DART boosting.

Functions:
    engineer_features -- add lag, rolling, and interaction features
    create_model      -- instantiate XGBRegressor with booster="dart"
    train_model       -- fit with optional early stopping
...
from dart.config import DARTDataConfig, DARTParams, TensorBoardConfig, log_system_metrics_to_tb
...
def create_model(
    params: DARTParams, seed: int = 42
) -> Tuple[XGBRegressor, dict]:
    """Create XGBRegressor with DART booster from config params."""
    kwargs = dict(
        booster="dart",
...
def train_model(
    ...
    y_test: pd.Series,
    fit_params: dict,
    params: Optional[DARTParams] = None,
    run_dir: Optional[Path] = None,
    tb_cfg: Optional[TensorBoardConfig] = None,
) -> XGBRegressor:
    """Fit DART model with optional TensorBoard logging."""
    if tb_cfg is None:
        tb_cfg = TensorBoardConfig()
    ...
    if tb_cfg.enabled and run_dir is not None:
        ...
        if tb_cfg.log_hparams_text and params is not None:
            hparam_text = (
                ...
                f"| rate_drop | {params.rate_drop} |\n"
                f"| skip_drop | {params.skip_drop} |\n"
                f"| sample_type | {params.sample_type} |\n"
                f"| normalize_type | {params.normalize_type} |\n"
